import { v4 } from "uuid";
import { incrementTimer } from "./xmltest";
...
const updateSectionTime = (section) => {
    let updatedSection = [];
    const offset = section[0][0].time;
...
export const setRepeater = (
    ...
    for (let i = 0; i < idx; i++) {
        ...
        if (song[i][0]?.repeaterCount === repeaterEndCount - 1) {
            ...
            const sectionTimeUpdated = updateSectionTime(section);
            ...
            const secondSectionTimeUpdated = updateSectionTime(secondSection);
            // console.log(
